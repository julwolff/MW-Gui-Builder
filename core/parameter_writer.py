...
def write_param_files(filein            = "simbox.xyz",
    ...
    with open(filein,'r') as file:
        ...
        for i in range (2, len(lines)):
            ...
            z.append(float(lines[i].split()[3]))
            
        cutoff = min(max(x),max(y),max(z))/2
    ...
    with open(fileout,"w") as file:
        ...
        if len(electrodes) > 0:
            ...
            file.write("# ----------------------\n")
            file.write("external_field\n")
            file.write("  type E                      # E stands for electric field\n")
            file.write("  direction 0.00 0.00 1.00    # Vector components along x, y and z\n")
            file.write("  amplitude {}       # Amplitude of the field (in a.u.)\n\n".format(amplitude))
