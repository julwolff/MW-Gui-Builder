...
class GuiParameterGenerator:
    ...
    def _build_input_tab(self):
        ...
        for i, mode in enumerate(["NVE", "NVT", "NPT"]):
            btn = ttk.Button(
                self.tab1,
                text=mode,
                command=lambda m=mode: self.toggle_sim_type(m)
            )
            btn.grid(row=3, column=1 + i, padx=5)

            self.sim_buttons[mode] = btn

        self.toggle_sim_type("NVT")
    ...
    def select_input_file(self):
        """Open a file browser and select the MetalWalls input file."""

        file_path = filedialog.askopenfilename(
            title="Select MetalWalls input file",
            filetypes=[
                ("MetalWalls input files", "*.inpt"),
                ("All files", "*.*"),
            ]
        )

        if file_path:
            self.filein_var.set(file_path)
    ...
    def launch(self):
        ...
        try:
            ...
            species = add_electrode_to_species(
                electrodes,
                species,
                filein
            )
            ...
            species, molecules = add_parameters(
                molecules,
                species,
                electrodes
            )
            ...
            messagebox.showinfo(
                "Success",
                "The parameter file was successfully generated."
            )
