...
A key step in running MetalWalls simulations is the generation of structure and input files that comply with the MetalWalls format, which encodes atomistic geometries, electrode definitions, and simulation parameters. While this format is powerful and flexible, manually creating or modifying these files can be time-consuming and error-prone, especially for complex geometries or for users less familiar with the internal structure of MetalWalls inputs. The most usual way consist in the used of a bulk creation software (like Packmol used in this case), and the addition of a structured electrode. However no codes can today adresses the correct creation of the parameter file specific to the MetalWalls software.
